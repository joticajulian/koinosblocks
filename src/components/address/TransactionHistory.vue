<template>
  <va-inner-loading :loading="loading" v-if="transactions.length">
    <va-card class="offset--sm row ma-3">
      ...
      <va-card-content>
        <div class="row">
          <div
              style="height: 400px;"
              class="flex md12 lg12"
          >
            <va-infinite-scroll
                :load="fetchTransactions"
            >
              <TransactionHistoryRow :row="row" v-for="row in transactions" :address="address"/>
            </va-infinite-scroll>
          </div>
        </div>
    ...
    const fetchTransactions = async () => {
      try {
        ...
        if (seq_num.value === null || seq_num.value > 0) {
          ...
          const {values} = await client.call('account_history', 'get_account_history', {
          ...
          if (values[values.length - 1].seq_num - 1 >= 0) {
            console.log('values[values.length - 1].seq_num - 1', values[values.length - 1].seq_num - 1);

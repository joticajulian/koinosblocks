      ...
      <va-card-content>
        <div v-if="block_topology">
          <DescriptionRow
            ...
            :data="prepareBlocks([block_topology.block_id])"
          />
          <DescriptionRow
            description="Block height"
            :data="prepareBlocks([block_topology.block_height])"
          />
          <DescriptionRow
            description="Created at"
            :data="toDateTime(block_topology.block.header.timestamp)"
          />
          <DescriptionRow
            description="Previous block"
            :data="prepareBlocks([block_topology.block.header.previous])"
          />
          <DescriptionRow
          ...
          <DescriptionRow
            ...
            :data="block_topology.receipt.disk_storage_used ?? '0'"
          />
          <DescriptionRow
          ...
          />
          <DescriptionRow
            description="Transaction merkle root"
            :data="block_topology.block.header.transaction_merkle_root"
          />
          <DescriptionRow
            description="Producer"
            :data="prepareProducers([block_topology.block.header.signer])"
          />
          <DescriptionRow
            description="Signature"
            :data="block_topology.block.signature"
          />
          <DescriptionRow
            v-if="block_topology.block.header.approved_proposals"
            description="Approved proposals"
            :data="
              prepareProposals(block_topology.block.header.approved_proposals)
            "
          />
        </div>
...
export default defineComponent({
  ...
  async setup(props) {
    ...
    return {
      ...
      prepareProducers: (producers: string[]): LinkedRow[] =>
        producers.map((p) => ({
          text: p,
          link: `/address/${p}`,
        })),
      prepareBlocks: (blocks: any[]): LinkedRow[] =>
        blocks.map((block: any) => ({
          text: block,
          link: `/block/${block}`,
        })),
      prepareProposals: (proposals: string[]): LinkedRow[] =>
        proposals.map((p) => ({
          text: p,
          link: `/proposal/${p}`,
        })),

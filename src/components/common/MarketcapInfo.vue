<template>
  <div v-if="data">
    <p>
      <strong>Price</strong>: ${{ data.current_price }}
  ...
  setup() {
    ...
    const getData = async () => {
      const response = await fetch(
        'https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=koinos&order=market_cap_desc&per_page=100&page=1&sparkline=false',
      );
